...
export class ProjectTableComponent implements OnInit {
    ...
    onGlobalFilter(event: Event) {
        ...
        const filteredFlatData = flatData.filter(
            (item) => item.employee.toLowerCase().includes(searchValue) || item.project.toLowerCase().includes(searchValue) || item.billing_status.toLowerCase().includes(searchValue) || item.total_hours.toString().toLowerCase().includes(searchValue)
        );
